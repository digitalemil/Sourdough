...
/usr/bin/loki -config.file=/opt/app/loki-local-config.yaml >/tmp/$LOGFOLDER/loki.log 2>&1  &

/usr/bin/tempo -config.file=/opt/app/tempo-local-config.yaml >/tmp/$LOGFOLDER/tempo.log 2>&1 &
...
./bin/grafana server >/tmp/$LOGFOLDER/grafana.log 2>&1 &
cd /opt/app
...
/opt/app/grafana-agent-linux-amd64  -config.expand-env -enable-features integrations-next --config.file /opt/app/config.yaml >/tmp/$LOGFOLDER/grafana-agent.log 2>&1  &
